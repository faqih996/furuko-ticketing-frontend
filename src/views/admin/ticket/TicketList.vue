...
// initialize ticket store and get necessary refs
const ticketStore = useTicketStore()
const { tickets } = storeToRefs(ticketStore)
const { fetchTickets } = ticketStore

// create filters ref with search field
const filters = ref({
  search: '',
  status: '',
  priority: '',
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div class="relative">
                  <input type="text" v-model="filters.search" placeholder="Cari tiket..."
                      class="w-full pl-10 pr-4 py-2 border border-gray-200 text-gray-500 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                  <i data-feather="search" class="w-4 h-4 text-gray-500 absolute left-3 top-2.5"></i>
              </div>
              <!-- status filters -->
              <select v-model="filters.status"
                  class="border border-gray-200 rounded-lg px-4 py-2 text-sm text-gray-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
          ...
          <table class="w-full">
              ...
              <tbody class="bg-white divide-y divide-gray-100">
                  <tr v-for="ticket in tickets" :key="ticket.code" class="hover:bg-gray-50">
                      ...
                      <td class="px-6 py-4 whitespace-nowrap">
                          <span
                              class="px-3 py-1 text-xs font-medium text-blue-700 bg-blue-100 rounded-full" :class="{
                      ...
                      <td class="px-6 py-4 whitespace-nowrap">
                          <span
                              class="px-3 py-1 text-xs font-medium text-gray-700 bg-gray-100 rounded-full" :class="{
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ DateTime.fromISO(ticket.created_at).toFormat('dd MMMM yyyy HH:mm') }}</td>
